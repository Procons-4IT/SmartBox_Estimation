Public Class clsBoMWizard_PO
    ...
    Private Sub PopulateEstimationsDetails(aform As SAPbouiCOM.Form, aChoice As String)
        ...
        If aChoice = "Header" Then
            ...
            oGrid.DataTable.ExecuteQuery("select  ' ' 'Select',T0.Code  from OITT T0 ")
        ...
        Else
            ...
            strItem = "SELECT T0.[DocEntry],T1.[DocNum], T0.[LineId],T0.[U_Z_ItemCode], T0.[U_Z_ItemDesc], T0.[U_Z_Spec],T0.[U_Z_Size] 'Size', T0.[U_Z_Price], T0.[U_Z_Qty], T0.[U_Z_Total], ' ' 'Select' FROM [dbo].[@Z_QUT1]  T0"
            strItem = strItem & " inner Join [@Z_OQUT] T1 on T1.DocEntry=T0.DocEntry where T0.[U_Z_ItemCode]<>'' AND T0.DocEntry in (" & strDocNumbers & ")"
            strItem = "select  ' ' 'Select',T0.Code,T1.Code 'ItemCode',T2.ItemName ,T1.Quantity ,T1.Warehouse 'WhsCode' ,T1.U_AVGCOST,T1.U_MARKUP,T1.Price from OITT T0 Inner Join ITT1 T1 on T1.Father=T0.Code inner Join OITM T2 on T2.ItemCode=T1.Code where T2.ItmsGrpCod<>112"
