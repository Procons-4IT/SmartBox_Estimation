...
Imports System.IO
Public Class clsProjectEstimation
    Inherits clsBase
    ...
    Public Sub LoadForm()
        Try
            oForm = oApplication.Utilities.LoadForm(xml_Estimation, frm_Estimation)
    ...
    Private Sub PopulateProjectPhaseCost(aCode As String, aRow As Integer)
        Dim oRec As SAPbobsCOM.Recordset
        Dim dblPrice, dblQty, dblMargin, dblTotalCost As Double
        dblPrice = 0
        dblQty = 1
        dblMargin = oApplication.Utilities.getMatrixValues(oMatrix, "V_3", aRow)
        oRec = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
        oRec.DoQuery("Select Sum(T1.""U_Z_TotalCost"") from ""@Z_OPRPH"" T0 inner Join ""@Z_PRPH1"" T1 on T1.""DocEntry""=T0.""DocEntry"" where T0.""U_Z_Code""='" & aCode & "'")
        oRec.DoQuery("Select Sum(T1.""U_Z_TotalCost""),sum(T1.""U_Z_Cost"") from ""@Z_OPRPH"" T0 inner Join ""@Z_PRPH1"" T1 on T1.""DocEntry""=T0.""DocEntry"" where T0.""U_Z_Code""='" & aCode & "'")

        If oRec.RecordCount > 0 Then
            dblTotalCost = oRec.Fields.Item(0).Value
            dblPrice = oRec.Fields.Item(1).Value
        End If
        oApplication.Utilities.SetMatrixValues(oMatrix, "V_4", aRow, dblPrice)
        oApplication.Utilities.SetMatrixValues(oMatrix, "V_2", aRow, dblQty)
        oApplication.Utilities.SetMatrixValues(oMatrix, "V_3", aRow, dblMargin)
        dblTotalCost = dblPrice + (dblPrice * dblMargin / 100)
        oApplication.Utilities.SetMatrixValues(oMatrix, "V_5", aRow, dblTotalCost)

    End Sub


    Private Sub CalculateTotalCost(aForm As SAPbouiCOM.Form, Optional aRow As Integer = 0)
        ...
        Try
            ...
            If aRow <> 0 Then
                ...
                intStart = aRow
            Else
                intEnd = oMatrix.RowCount
            End If
            For intRow As Integer = intStart To intEnd
                aRow = intRow
                dblUnitPrice = oApplication.Utilities.getMatrixValues(oMatrix, "V_4", aRow)
                dblQuantity = oApplication.Utilities.getMatrixValues(oMatrix, "V_2", aRow)
                dblMargin = oApplication.Utilities.getMatrixValues(oMatrix, "V_3", aRow)
                dblUnitPrice = dblUnitPrice + (dblUnitPrice * dblMargin / 100)
                dblUnitPrice = dblUnitPrice * dblQuantity
                oApplication.Utilities.SetMatrixValues(oMatrix, "V_5", aRow, dblUnitPrice)
            Next
            aForm.Freeze(False)
        Catch ex As Exception
            oApplication.Utilities.Message(ex.Message, SAPbouiCOM.BoStatusBarMessageType.smt_Error)
...
#Region "Item Event"
    Public Overrides Sub ItemEvent(ByVal FormUID As String, ByRef pVal As SAPbouiCOM.ItemEvent, ByRef BubbleEvent As Boolean)
        Try
            If pVal.FormTypeEx = frm_Estimation Then
                    ...
                    Case True
                        Select Case pVal.EventType
                            ...
                            Case SAPbouiCOM.BoEventTypes.et_CLICK
                                If (pVal.ItemUID = "17" Or pVal.ItemUID = "100" Or pVal.ItemUID = "101") And pVal.Row > 0 Then
                            ...
                            Case SAPbouiCOM.BoEventTypes.et_MATRIX_LINK_PRESSED
                                ...
                                If pVal.ItemUID = "17" And pVal.ColUID = "V_0" Then
                                    ...
                                    oobj.LoadForm_View(oApplication.Utilities.getMatrixValues(oMatrix, "V_0", pVal.Row))
                                    BubbleEvent = False
                                    Exit Sub
                                End If
                        End Select
                    Case False
                        Select Case pVal.EventType
                            ...
                            Case SAPbouiCOM.BoEventTypes.et_KEY_DOWN
                                oForm = oApplication.SBO_Application.Forms.Item(FormUID)
                                If pVal.ItemUID = "17" And (pVal.ColUID = "V_2" Or pVal.ColUID = "V_4" Or pVal.ColUID = "V_3") And pVal.CharPressed = 9 Then
                                    ' oForm.Freeze(True)
                                    Dim dblUnitPrice, dblQuantity, dblMargin As Double
                                    oMatrix = oForm.Items.Item("17").Specific
                                    ...
                                    dblMargin = oApplication.Utilities.getMatrixValues(oMatrix, "V_3", pVal.Row)
                                    dblUnitPrice = dblUnitPrice + (dblUnitPrice * dblMargin / 100)
                                    dblUnitPrice = dblUnitPrice * dblQuantity
                                    dblUnitPrice = Math.Round(dblUnitPrice, 3)
                                    oApplication.Utilities.SetMatrixValues(oMatrix, "V_5", pVal.Row, dblUnitPrice)
                                    ' oForm.Freeze(False)
                                End If
                                ...
                                Try
                                    ...
                                    If Not oForm.Mode = SAPbouiCOM.BoFormMode.fm_FIND_MODE Then
                                        ...
                                        If pVal.ItemUID = "41" Then
                                            ...
                                            Try
                                                Val2 = oDataTable.GetValue("U_Z_CardCode", 0)
                                            Catch ex As Exception
                                                Val2 = oDataTable.GetValue("U_Z_CARDCODE", 0)
                                            End Try
                                            ...
                                            Try
                                                Try
                                                    oApplication.Utilities.setEdittextvalue(oForm, "30", Val2)
                                                Catch ex As Exception

                                                End Try

                                                Try
                                                    oApplication.Utilities.setEdittextvalue(oForm, "36", oDataTable.GetValue("U_Z_CardName", 0))
                                                Catch ex As Exception
                                                    oApplication.Utilities.setEdittextvalue(oForm, "36", oDataTable.GetValue("U_Z_CARDNAME", 0))
                                                End Try
                                        ...
                                        If pVal.ItemUID = "44" Then
                                            ...
                                            Val2 = oDataTable.GetValue("U_Z_GLAcc", 0)
                                            Try
                                                oApplication.Utilities.setEdittextvalue(oForm, "47", Val2)
                                                oApplication.Utilities.setEdittextvalue(oForm, "45", val1)
                                                oApplication.Utilities.setEdittextvalue(oForm, pVal.ItemUID, val)
                                            Catch ex As Exception
                                                If oForm.Mode = SAPbouiCOM.BoFormMode.fm_OK_MODE Then
                                                    oForm.Mode = SAPbouiCOM.BoFormMode.fm_UPDATE_MODE
                                        ...
                                        If pVal.ItemUID = "17" And pVal.ColUID = "V_0" Then
                                            ...
                                            oMatrix = oForm.Items.Item(pVal.ItemUID).Specific
                                            Try
                                                ' oApplication.Utilities.SetMatrixValues(oMatrix, "V_4", pVal.Row, oDataTable.GetValue("U_Z_UnitPrice", 0))
                                                oApplication.Utilities.SetMatrixValues(oMatrix, "V_3", pVal.Row, oDataTable.GetValue("U_Z_Margin", 0))
                                            ...
                                            Dim dblUnitPrice, dblQuantity As Double
                                            PopulateProjectPhaseCost(val, pVal.Row)
                                        End If

                                        If pVal.ItemUID = "17" And pVal.ColUID = "V_12" Then
                                            val = oDataTable.GetValue("U_Z_Code", 0)
                                            '  val1 = oDataTable.GetValue("U_Z_Name", 0)
                                            oMatrix = oForm.Items.Item(pVal.ItemUID).Specific
                                            Try
...
#End Region
    ...
    Private Sub EnableControls(aform As SAPbouiCOM.Form, aFlag As Boolean)
        ...
        aform.Items.Item("24").Click(SAPbouiCOM.BoCellClickType.ct_Regular)
        For intRow As Integer = 0 To aform.Items.Count - 1
            If aform.Items.Item(intRow).UniqueID <> "24" And aform.Items.Item(intRow).Type <> SAPbouiCOM.BoFormItemTypes.it_FOLDER Then
                aform.Items.Item(intRow).Enabled = aFlag
            End If
        Next
        aform.Items.Item("1").Enabled = True
        aform.Items.Item("2").Enabled = True
            ...
            Select Case pVal.MenuUID
                ...
                Case "1287"
                    ...
                    If pVal.BeforeAction = False Then
                        ...
                        oApplication.Utilities.setEdittextvalue(oForm, "42", "")
                        oApplication.Utilities.setEdittextvalue(oForm, "30", "")
                        oApplication.Utilities.setEdittextvalue(oForm, "36", "")
                        oApplication.Utilities.setEdittextvalue(oForm, "44", "")
...
#Region "Data Event"
    Public Sub FormDataEvent(ByRef BusinessObjectInfo As SAPbouiCOM.BusinessObjectInfo, ByRef BubbleEvent As Boolean)
        Try
            If BusinessObjectInfo.BeforeAction = False And (BusinessObjectInfo.EventType = SAPbouiCOM.BoEventTypes.et_FORM_DATA_LOAD) Then
                oForm = oApplication.SBO_Application.Forms.ActiveForm()
                If oForm.TypeEx = frm_Estimation Then
                    oForm.Items.Item("24").Click(SAPbouiCOM.BoCellClickType.ct_Regular)
                    oDataSrc_Line = oForm.DataSources.DBDataSources.Item("@Z_OQUT")
                    If oDataSrc_Line.GetValue("U_Z_DocStatus", 0).Trim = "P" Then
                        EnableControls(oForm, True)
                    Else
            ...
            If BusinessObjectInfo.BeforeAction = False And BusinessObjectInfo.ActionSuccess = True And (BusinessObjectInfo.EventType = SAPbouiCOM.BoEventTypes.et_FORM_DATA_ADD Or BusinessObjectInfo.EventType = SAPbouiCOM.BoEventTypes.et_FORM_DATA_UPDATE) Then
                ...
                If oForm.TypeEx = frm_Estimation Then
                    ...
                    stXML = stXML.Replace("<?xml version=""1.0"" encoding=""UTF-16"" ?><Project_EstimationParams><DocEntry>", "")
                    stXML = stXML.Replace("</DocEntry></Project_EstimationParams>", "")
                    ...
                    If stXML <> "" Then
                        ...
                        If otest.RecordCount > 0 Then
                            'If 1 = 1 Then ' otest.Fields.Item("U_Z_DocStatus").Value = "R" Then
                            '    strQuery = "Select Sum(""U_Z_Total"") from ""@Z_QUT1"" where ""DocEntry""=" & stXML
                            '    otest.DoQuery(strQuery)
                            '    If otest.RecordCount > 0 Then
                            '        strQuery = "Update ""@Z_OQUT"" set ""U_Z_TotalCost""='" & otest.Fields.Item(0).Value & "' where ""DocEntry""=" & stXML
                            '        otest.DoQuery(strQuery)
                            '    End If

                            '    strQuery = "Update ""@Z_OQUT"" set ""U_Z_AppStatus""='A',""U_Z_DocStatus""='A' where ""DocEntry""='" & stXML & "'"
                            '    otest.DoQuery(strQuery)
                            'End If
                            If otest.Fields.Item("U_Z_DocStatus").Value = "R" Then
                    ...
                    Else
                        ...
                        If otest.RecordCount > 0 Then
                            ...
                            otest.DoQuery(strQuery)
                        End If
                        strQuery = "Update ""@Z_OQUT"" set ""U_Z_AppStatus""='A',""U_Z_DocStatus""='A' where ""U_Z_DocStatus""='R'  and ""DocEntry""=" & stXML
                        otest.DoQuery(strQuery)
                    End If
                End If
    ...
    Public Sub UpdateApprovalRequired(ByVal strTable As String, ByVal sColumn As String, ByVal StrCode As String, ByVal ReqValue As String, ByVal AppTempId As String)
        Try
            ...
            oRecordSet = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
            If blnIsHana = True Then
                strQuery = "Update """ & strTable & """ set ""U_Z_AppID""='" & AppTempId & "', ""U_Z_AppRequired""='" & ReqValue & "',""U_Z_AppReqDate"" = NOW()"
            Else
                strQuery = "Update """ & strTable & """ set ""U_Z_AppID""='" & AppTempId & "', ""U_Z_AppRequired""='" & ReqValue & "',""U_Z_AppReqDate"" = getdate()"
            End If
...
#Region "Validations"
    ...
    Private Function Validation_Form(ByVal oForm As SAPbouiCOM.Form) As Boolean
        ...
        Try
            oCombobox = oForm.Items.Item("8").Specific
            If oApplication.Utilities.getEditTextvalue(oForm, "41") = "" Then
            ...
            If oApplication.Utilities.getEditTextvalue(oForm, "44") = "" Then
                oApplication.Utilities.Message("Sub Project Detail is missing...", SAPbouiCOM.BoStatusBarMessageType.smt_Error)
                Return False
            End If
...
#Region "Function"
    ...
    Private Sub RefereshDeleteRow(ByVal aForm As SAPbouiCOM.Form)
        Try
            ...
            ElseIf aForm.PaneLevel = 4 Then
                oMatrix = aForm.Items.Item("101").Specific
                oDBDataSourceLineZ_1 = oForm.DataSources.DBDataSources.Item("@Z_QUT3")
            End If
            '   oMatrix = aForm.Items.Item("17").Specific
