Public Class clsBoMSummary
    ...
    Private Sub DataBind(aForm As SAPbouiCOM.Form)
        ...
        oRec.DoQuery("Select * from ""@Z_OITT"" where ""U_Z_ItemCode""='" & oApplication.Utilities.getEditTextvalue(aForm, "4") & "'")
        ...
        oGrid.DataTable.ExecuteQuery("Select * from ""@Z_OITT"" where ""U_Z_ItemCode""='" & oApplication.Utilities.getEditTextvalue(aForm, "4") & "'")
        FormatGrid(oGrid)
    ...
    Private Sub FormatGrid(aGrid As SAPbouiCOM.Grid)
        ...
        oEditTextColumn.ColumnSetting.SumType = SAPbouiCOM.BoColumnSumType.bst_Auto
        aGrid.Columns.Item("U_Z_AvgMarkUp").Editable = False
    ...
    Private Function AddtoUDT(ByVal aform As SAPbouiCOM.Form, ByVal ItemCode As String, aChoice As String) As Boolean
        ...
        oUserTable = oApplication.Company.UserTables.Item("Z_OITT")
        If aChoice = "Add" Then
        ...
        Else
            ...
            For intRow As Integer = 0 To oGrid.DataTable.Rows.Count - 1
                ...
                oUserTable.UserFields.Fields.Item("U_Z_AvgMarkUp").Value = oGrid.DataTable.GetValue("U_Z_AvgMarkUp", intRow)
                oUserTable.Update()
            Next
            UpdateBom(ItemCode)
        End If
    ...
    Private Function AddtoUDT_Initialize(ByVal ItemCode As String, aChoice As String) As Boolean
        ...
        oUserTable = oApplication.Company.UserTables.Item("Z_OITT")
        If aChoice = "Add" Then
            strCode = oApplication.Utilities.getMaxCode("@Z_OITT", "Code")
            ...
            otemp1.DoQuery("Select Sum(U_AVGCOST *Quantity), from ITT1 T0  Inner Join  OITM T1 on T1.ItemCode=T0.Code  INNER JOIN OITB T2 ON T1.[ItmsGrpCod] = T2.[ItmsGrpCod] where Father='" & ItemCode & "' and Type=4 and T1.ItmsGrpCod<>112")

            oUserTable.UserFields.Fields.Item("U_Z_Cost").Value = otemp1.Fields.Item(0).Value
            oUserTable.UserFields.Fields.Item("U_Z_Markup").Value = 1
            ...
            strTemp = "SELECT Sum(U_AVGCOST *Quantity)   FROM ITT1 T0  inner Join  ORSC T1 on T1.VisResCode=T0.Code where Father='" & ItemCode & "' and T0.Type=290 and T1.ResGrpCod =(" & strTemp & ")"
            ' otemp1.DoQuery("Select Sum(U_AVGCOST) from ITT1 T0 where Father='" & ItemCode & "' and Type=4")
    ...
    Private Function AddtoUDT_Initialize_Update(ByVal ItemCode As String, aChoice As String) As Boolean
        ...
        If aChoice = "Update" Then
            otemp1.DoQuery("Select Code,* from [@Z_OITT] where U_Z_ItemCode='" & ItemCode & "' and U_Z_Type='Material'")
            If otemp1.RecordCount > 0 Then
                ...
                otemp1.DoQuery("Select Sum(U_AVGCOST*Quantity),Avg(U_MARKUP) from ITT1 T0  Inner Join  OITM T1 on T1.ItemCode=T0.Code  INNER JOIN OITB T2 ON T1.[ItmsGrpCod] = T2.[ItmsGrpCod] where Father='" & ItemCode & "' and Type=4 and T1.ItmsGrpCod<>112")
                ...
                oUserTable.UserFields.Fields.Item("U_Z_AvgMarkUp").Value = otemp1.Fields.Item(1).Value
                oUserTable.Update()
            End If
            Dim intResourceCode As Integer
            Dim strTemp As String

            otemp1.DoQuery("Select Code,* from [@Z_OITT] where U_Z_ItemCode='" & ItemCode & "' and U_Z_Type='Labor'")
            If otemp1.RecordCount > 0 Then
                strCode = otemp1.Fields.Item("Code").Value
                oUserTable.GetByKey(strCode)
                oUserTable.Name = strCode
                ...
                oUserTable.UserFields.Fields.Item("U_Z_Markup").Value = otemp1.Fields.Item("U_Z_Markup").Value
                strTemp = "SELECT  Top 1 T1.[ResGrpCod] FROM ORSC T0  INNER JOIN ORSB T1 ON T0.[ResGrpCod] = T1.[ResGrpCod] where T1.[ResGrpNam] like 'Labour%'"
                strTemp = "SELECT Sum(U_AVGCOST*Quantity) ,Avg(U_MARKUP)  FROM ITT1 T0  inner Join  ORSC T1 on T1.VisResCode=T0.Code where Father='" & ItemCode & "' and T0.Type=290 and T1.ResGrpCod =(" & strTemp & ")"
                otemp1.DoQuery(strTemp)
                dblAvgCost = otemp1.Fields.Item(0).Value
            ...
            otemp1.DoQuery("Select Code,* from [@Z_OITT] where U_Z_ItemCode='" & ItemCode & "' and U_Z_Type='Machine'")
            If otemp1.RecordCount > 0 Then
                strCode = otemp1.Fields.Item("Code").Value
                oUserTable.GetByKey(strCode)
                oUserTable.Name = strCode
            ...
            otemp1.DoQuery("Select Code,* from [@Z_OITT] where U_Z_ItemCode='" & ItemCode & "' and U_Z_Type='OutSource'")
            If otemp1.RecordCount > 0 Then
                strCode = otemp1.Fields.Item("Code").Value
                oUserTable.GetByKey(strCode)
                oUserTable.Name = strCode
            ...
            otemp1.DoQuery("Select Code,* from [@Z_OITT] where U_Z_ItemCode='" & ItemCode & "' and U_Z_Type='LED Material'")
            If otemp1.RecordCount > 0 Then
                strCode = otemp1.Fields.Item("Code").Value
                oUserTable.GetByKey(strCode)
                oUserTable.Name = strCode
            ...
            'oUserTable.UserFields.Fields.Item("U_Z_ItemCode").Value = ItemCode
            'oUserTable.UserFields.Fields.Item("U_Z_Type").Value = "LED Material"
            'oUserTable.UserFields.Fields.Item("U_Z_Cost").Value = 0
            'oUserTable.UserFields.Fields.Item("U_Z_Markup").Value = 0
            'oUserTable.UserFields.Fields.Item("U_Z_Price").Value = 0
            'oUserTable.Add()
        End If
        otemp1.DoQuery("Update [@Z_OITT] set U_Z_Price=U_Z_Cost * U_Z_Markup where U_Z_ItemCode='" & ItemCode & "'")
    ...
    Private Sub UpdateBom(aCode As String)
        ...
        oTemp = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
        otemp1 = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
        oTemp.DoQuery("Select * from [@Z_OITT] where U_Z_ItemCode='" & aCode & "' order by Code")
                            ...
                            Case SAPbouiCOM.BoEventTypes.et_ITEM_PRESSED
                                ...
                                If pVal.ItemUID = "3" Then
                                    If AddtoUDT(oForm, oApplication.Utilities.getEditTextvalue(oForm, "4"), "Update") = True Then
                                        ...
                                        frm_SourceBoM.Items.Item("25").Click()
                                        If frm_SourceBoM.Mode = SAPbouiCOM.BoFormMode.fm_UPDATE_MODE Then
                                            frm_SourceBoM.Items.Item("1").Click()
                                        End If
                                    End If
                                End If
